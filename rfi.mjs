...
export function installRfi({engine,getProject,mutate,schedule,toast,navigate}){
 ...
 $('rfi-grid').oninput=()=>{const item=chosen();if(item){item.gridLine=$('rfi-grid').value;registerDock.refresh();schedule();}};
 $('rfi-question').oninput=()=>{const item=chosen();if(item){item.question=$('rfi-question').value;registerDock.refresh();schedule();}};
